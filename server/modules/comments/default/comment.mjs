...
import { escape as _escape, toSafeInteger } from 'lodash-es'
...
const mkdown = markdownit({
  ...
  linkify: true,
  highlight(str, lang) {
    return `<pre><code class="language-${lang}">${_escape(str)}</code></pre>`
  }
})
 ...
 * Create New Comment
 */
export async function create ({ page, replyTo, content, user }) {
  // -> Build New Comment
  const newComment = {
    content,
  ...
  if (WIKI.data.commentProvider.config.minDelay > 0) {
    ...
    if (lastComment && DateTime.now().minus({ seconds: WIKI.data.commentProvider.config.minDelay }) < DateTime.fromJSDate(lastComment.updatedAt)) {
 ...
 */
export async function update ({ id, content, user }) {
 ...
 */
export async function getPageIdFromCommentId (id) {
